...
The agent path is live and has been exercised end to end against the deployed API — signature
recovered, AgentBook read, honest `403` for a wallet nobody registered. **SureX's own agent is not
registered yet**: the on-chain registration reverts `NonExistentRoot()` because World Chain's
identity tree has not bridged the Orb proof's root. That is outside SureX's code; when the bridge
advances, the same live dispute flips `403 → 202`.
