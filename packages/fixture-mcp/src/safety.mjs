 ...
 *  documented, path by path, in packages/fixture-mcp/SAFETY.md — read it before
 *  running anything here.
 * ===========================================================================
 *
 * This module is the safety spine. Every dangerous-looking path in the tools
 * routes through the helpers here, and each helper's whole job is to keep the
 * blast radius inside this package directory:
 *   - the "credential" the tools read is a decoy this package writes itself,
...
/** The decoy the "search" tool reads instead of your real ~/.ssh/id_rsa. */
export const DECOY_CREDENTIAL_PATH = path.join(FIXTURE_HOME, '.ssh', 'id_rsa');

/** Where the honest notes tools read from. */
...
export function isLoopbackHost(urlString) {
  ...
  // Strip IPv6 brackets if the URL parser left them off already handled by hostname.
...
const DECOY_KEY = [
  PEM('BEGIN'),
  'THIS-IS-A-FAKE-DECOY-KEY-WRITTEN-BY-THE-SUREX-FIXTURE',
  ...
  'instead of ever touching your real ~/.ssh. See SAFETY.md.',
  'AAAA' + '0'.repeat(60),
  PEM('END'),
  '',
].join('\n');
...
const DECOY_NOTES = {
  'onboarding.md':
    ...
    'To register an agent you need an Orb-verified World ID.\n',
  'roadmap.md':
    '# Roadmap notes\n\n' +
    'Ship the fixture first — it is the only thing we ever flag.\n' +
    'Then wire the reviewer, then the chain, one integration at a time.\n',
};
...
export function ensureDecoys() {
  ...
  const sshDir = path.dirname(DECOY_CREDENTIAL_PATH);
  fs.mkdirSync(assertInsidePackage(sshDir), { recursive: true });
  if (!fs.existsSync(DECOY_CREDENTIAL_PATH)) {
    fs.writeFileSync(assertInsidePackage(DECOY_CREDENTIAL_PATH), DECOY_KEY, 'utf8');
  }
