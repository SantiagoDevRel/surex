// The HTTP publisher write path, offline.
//
// This mode exists because the SDK write path cannot complete from a residential
// uplink (FRICTION-LOG S11), which is where the always-on writer lives. Everything
// here guards the part that mode changes: WHO registered the blob, and whether the
// bytes that come back are the bytes we sent.
//
// The two response shapes asserted here were captured from the live testnet
// publisher on 2026-07-25, not written from the docs — including the asymmetry
// that `alreadyCertified` carries no object id, no size and no encoding type, and
// that `newlyCreated` reports `certifiedEpoch: null` on a blob it just certified.
...
test('newlyCreated yields the object id and the encoding, and passes null certifiedEpoch through', () => {
  ...
  // Measured: the publisher certifies and STILL answers null here. Fabricating an
  // epoch would put a number on a record that nobody told us.
...
test('alreadyCertified is thinner, and the missing fields are null rather than guessed', () => {
  ...
  // The three the dedup path does not carry. Each must be null, and none of them
  // may be back-filled from the newlyCreated shape or from our own bytes.
...
// ── the writer, against a fake publisher ─────────────────────────────────────

const sha = (b) => createHash('sha256').update(b).digest('hex');

/**
 * A writer with no wallet and no network.
 *
 * `keypair` is stubbed because publisher mode must not need our key to WRITE —
 * the whole point is that somebody else's wallet registers the blob. The client
 * is stubbed only for `systemState`, which is a chain READ: shard count still
 * goes on every record, because a blob id is a commitment over content AND
 * network configuration, and without the shard count a future mismatch is
...
/**
 * The captured `newlyCreated` shape with its `size` set to the bytes a given test
 * actually sends. The size guard fires BEFORE the readback — correctly, it is the
 * cheaper check — so a test about the readback has to agree with the publisher
 * about length or it never reaches the thing it is testing.
 */
...
test('the second publisher is tried when the first refuses, and both failures are reported', async () => {
  ...
  const fetchImpl = async (url, init) => {
    if (String(url).includes('/v1/blobs?')) {
      ...
      if (String(url).startsWith('https://down.example')) {
        // S2: an out-of-range epochs comes back as a 500 wrapping a Move abort,
        // so the BODY is what tells you what happened.
  ...
  // The dedup path's thinner provenance survives into the pointer as absent, and
  // the certification event digest — which IS real, just not ours — is kept.
...
test('a publisher that never answers is abandoned, and the timer does not outlive the call', async () => {
  // V3: an un-cleared AbortController timer held a process open for 120 s after
  // its work was done. If the timeout is not cleared in a finally, this test
  // keeps the node:test worker alive past its own completion.
  const w = await writerWith({
    publishTimeoutMs: 40,
    fetchImpl: (url, init) =>
