 ...
 *
 * 1. Seeded entries are `unknown`. NEVER `clean`. A seeded entry that inherits an
 *    existing backdoor must not gain legitimacy from being listed, and the
 *    registry's whole value is that being in it is not an endorsement.
 *    entities.mjs enforces it too: buildVerdictHead refuses `clean` without a
 *    review key.
 *
 * 2. The ~50 seed-time RegistryEntry bodies go into ONE Walrus Quilt.
 *    A standalone blob is two Sui transactions; 50 of them is 100, and the
 *    testnet faucet took 53 blind attempts to answer once (FRICTION-LOG S1), so
 *    transaction count is the real budget. Quilt batches up to 660 small blobs
 *    into one storage unit — measured here as 2 transactions total for the whole
 *    seed. Standalone certified blobs stay for source trees, reviews and dispute
 *    evidence, where per-record citability is the entire point.
 *
 *    The trade is recorded, not hidden: a quilted record is addressed as
 *    (quilt blob, patch id) and has NO certified Sui object of its own, so it has
 *    no per-record explorer link. Every pointer written by this script carries
 *    `addressing: 'quilt-patch'`, its own `patchId`, and the quilt's digests, so a
 *    reader can see which kind of record they are looking at.
 */

// @surex/core and @surex/worker are imported by RELATIVE path, not by package
// name: the repo root is a private workspace container with no dependencies of
// its own, so pnpm links neither into the root node_modules. `viem` and
// `@arkiv-network/sdk` DO resolve by name — node-linker=hoisted puts them at the
...
/**
 * The disclosure that goes on every seeded record. Written by us, so it is held to
 * the copy law: the word is REVIEWED, and a listing is never an endorsement.
 */
...
// ─── verify-only path ────────────────────────────────────────────────────────
//
// Reads only what a consumer can read — the `.createdBy(writer)` scoped query —
// and then follows every evidence pointer to the bytes and hashes them. The
// counts alone would prove nothing: 50 rows each pointing at the wrong record
// also counts to 50.
if (VERIFY_ONLY) {
...
// ─── 1 · candidates (frozen on first run so a resume seeds the SAME set) ─────
step('1 · candidates');
...
// ─── 2 · licence gate, per candidate, checkpointed after each ───────────────
...
// ─── 3 · ONE quilt with every RegistryEntry body ────────────────────────────
step('3 · Walrus — one quilt, every seed RegistryEntry body');
/**
 * The record body. Must be BYTE-DETERMINISTIC for a given candidate, or the
 * digest recorded on the Arkiv entity cannot be re-derived and every later check
 * degrades to "trust the store". So `capturedAt` is minted once per server and
 * persisted in the checkpoint rather than being `new Date()` at call time.
 */
function entryBody(candidate) {
  ...
  return {
    ...
    // Recorded as an alias, and explicitly marked as having no entity yet, so
    // nobody reads this as a claim that it is in the registry.
    ...
    // npm dist.integrity, recorded now because it is free now and unobtainable
    // once a version is unpublished. It is what makes Tier A reachable later.
...
// ─── 3b · repair a mis-recorded identifier → patch-id mapping ───────────────
//
// `flow.listFiles()` returns patch ids with NO identifier and NOT in input order
// (measured: positional mapping was right for 1 of 50). A seed written before that
// was known has 50 entries each pointing at another entry's bytes. This re-derives
// the mapping from the certified quilt itself — the identifier comes from the
// quilt index and the digest from the bytes the quilt serves — and rewrites the
// affected registryEntry entities. It does NOT write a new quilt: the bytes are
// already certified, and re-writing them would re-charge (S3).
if (REPAIR_POINTERS) {
  ...
  // Cross-check the mapping against the record's OWN content, not just the index:
  // each body carries its fingerprint, and it must equal the identifier the quilt
  // gave the patch. If those two ever disagree, the mapping is not trustworthy and
...
// ─── 4 · Arkiv — RegistryEntry + VerdictHead per server ─────────────────────
...
// ─── 5 · read back the way a consumer will ──────────────────────────────────
