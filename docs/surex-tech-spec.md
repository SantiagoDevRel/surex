...
3. Every review runs **twice with paraphrased prompts**. Agreement → verdict stands (where the two severities differ, the lower one wins). **Disagreement buys one more reading of *each* prompt** — a balanced panel of four — and the majority decides; with no majority the verdict is `unreviewable` with reason `no-agreement`, and `agreementRuns` records the size of the majority. A `clean` majority does not carry the dissenting reading's findings, which would be a contradiction the schema rejects; the dissent is recorded as a note and every reading's raw output goes into the evidence blob.

   > **Why one more of each and not simply a third.** With readings {a₁, b₁, a₂} the tie-breaking draw comes from variant `a`'s own distribution and agrees with a₁ more often than with b₁ for reasons unrelated to the code — the split would resolve toward whichever prompt happened to be re-run. A balanced {a₁, b₁, a₂, b₂} has no such tilt, and it earns the 2-2 case: when two prompts persistently disagree there is no majority, and "these readings do not converge" is the honest answer a single tie-break would have papered over with a verdict.

   > **Corrected against measurement, 2026-07-25.** This rule used to say "disagreement → severity capped and `agreementRuns` recorded", which in practice kept the more *accusatory* side of a split at severity 2. Calibration over the sixteen fixtures, three runs each (`scripts/calibrate.mjs --runs 3`), showed why that is wrong: `honest-sqlite` returned **flagged, clean, clean** on three identical inputs, while all fifteen other fixtures were stable. Every correct verdict in the set came back with `agreementRuns: 2`; the only incorrect one was the only one with `agreementRuns: 1`. The split was sampling noise, and the old rule turned it into a published accusation. Note what the change does *not* alter: `decide()` answers `warn` both for a severity-2 flag and for `unreviewable`, so the gate behaves identically — what changes is whether the registry makes a claim about somebody's code that it cannot support.
