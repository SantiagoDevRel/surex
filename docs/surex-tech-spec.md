...
1. **Blob-ID reproducibility is still unsolved — but no longer blocks Tier A.** Tarballs aren't deterministic (gzip mtimes, OS byte, file ordering), and a blob ID can't be computed with a standard hash function (§5). Tier A now runs off npm `dist.integrity` instead, which sidesteps both. Still worth settling for the archive itself: normalise the uploaded tree (sorted paths, zeroed mtimes) and record a `normalisedTreeSha256` so re-uploads are comparable.
